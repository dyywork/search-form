    ...
    getHideData () {
      ...
      this.formItemList.forEach((formItem, index) => {
        if (index > this.firstIndex) {
          Object.keys(this.form).forEach(key => {
            if (formItem.model === key && this.form[key] !== '' && this.form[key] !== null && this.form[key].length > 0) {
      ...
      this.formItemList.forEach((formItem) => {
        ...
        if (isHideForm.length > 0) {
          Object.keys(this.form).forEach(key => {
            if (formItem.model === key && this.form[key] !== '' && this.form[key] !== null && (this.form[key].length > 0 || typeof this.form[key] === 'number')) {
              if (formItem.type === 'input') {
                ...
                console.log(formItem.initialValue)
